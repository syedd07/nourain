    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-shadow duration-300 py-6 ${isScrolled ? 'bg-white shadow-lg' : 'bg-[#f8fafc] shadow-md'}`}>
      ...
      <div
        ...
        className={`fixed top-0 left-0 h-full w-1/2 bg-white shadow-lg transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
          } transition-transform duration-300 ease-in-out lg:hidden z-50 flex flex-col justify-between`}
      >
        <div>
          {/* Close Button */}
          <button
        ...
        {/* Mobile Footer: This appears only when mobile menu is open */}
        <div className="px-6 py-4 border-t">
          <p className="text-sm text-gray-500">
            © {new Date().getFullYear()}
            <span className="text-[#007bff] hover:underline"> Al-Nourain. </span>
            All rights reserved.
          </p>
          <span className="text-sm text-gray-500">
            Designed by{" "}
            <a
              href="https://sydali.me"
